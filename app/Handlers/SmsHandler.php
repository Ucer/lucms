...
require_once dirname(__DIR__) . '/Extends/aliyun-dysms-php-sdk/api_sdk/vendor/autoload.php';
...
class Sms
{
    ...
    {
        $this->config = \Illuminate\Support\Facades\Config::get('services.aliyun_sms');
    }
    ...
    public  function getAcsClient()
    {
        ...
        $accessKeyId = $this->config['account']['access_key_id']; // AccessKeyId

        $accessKeySecret = $this->config['account']['access_key_secret']; // AccessKeySecret
     ...
     * @return stdClass
     */
    public  function sendSms($phone, $variables)
    {
        ...
        $request->setSignName($this->config['template']['sign_name']);

        // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
        $request->setTemplateCode($this->config['template']['template_code']);

        // 可选，设置模板参数, 假如模板中存在变量需要替换则为必填项
        $request->setTemplateParam(json_encode($variables, JSON_UNESCAPED_UNICODE));
